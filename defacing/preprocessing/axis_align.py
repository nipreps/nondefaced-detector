...
def align(template, volume):
    ...
    for ix in list(range(3)):
        ...
        for jy in axis:
            axis = list(range(3))
            axis.pop(ix); axis.pop(jy)
            axis_combination.append((ix, jy, axis[0]))
            errors.append(np.linalg.norm(template - volume.transpose(axis_combination[-1])))
    i = np.argmin(errors)

    return (transformed_volume.transpose(axis_combination[i]), 
            axis_combination[i])
