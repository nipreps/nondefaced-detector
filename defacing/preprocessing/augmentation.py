    ...
    def __init__(self, DISTRIBUTION, augmentations = None):
        ...
        for key in augmentations.keys():
            if key.lower() not in allowed:
                raise ValueError("{} following augmentation not allowed. \n\
                                    Allowed augmentations are: {}".format(key, allowed))
            ...
            if p < self.augmentations[key]:
                if key.lower() == 'rotation':
                    ...
                    self.distribution = np.rot90(self.TRUEDISTRIBUTION, k)
                     ...
                     matrix = get_affine(
                        volume_shape=np.asarray(x.shape, 
                                        translation=translation)
                    )
                    x, self.distribution = warp_features_labels(features=x, 
                                                        labels=self.distribution, 
                                                        matrix=matrix)
