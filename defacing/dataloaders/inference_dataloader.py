...
class DataGeneratoronFly(object):
	...
	def get_data(self, data):
		...
		data = self.coreg.register_patient(data, self.fixed_image).astype('float64')
	...
	def _get_random_slices(self, volume):
		...
		x = np.random.randint(dimensions[0]//5, 4*dimensions[0]//5, self.nmontecarlo)
		z = np.random.randint(dimensions[1]//5, 4*dimensions[1]//5, self.nmontecarlo)
		y = np.random.randint(dimensions[2]//5, 4*dimensions[2]//5, self.nmontecarlo)
